...
type MapboxFeature = SearchResult & {
  geometry?: {
...
const MAPBOX_CSS = "https://api.mapbox.com/mapbox-gl-js/v3.9.4/mapbox-gl.css";
...
function sanitizeDiagnosticValue(
  ...
  return value
    ...
    .replace(
      /([?&](?:access_token|token|mapbox_token)=)[^&\s]+/gi,
      "$1[redacted]",
    );
}
...
function App() {
  ...
  const [performanceMode, setPerformanceMode] = useStoredState<PerformanceMode>(
    "ride-performance",
    "balanced",
  );
  ...
  const [mapboxTokenSource, setMapboxTokenSource] = useState<MapboxTokenSource>(
    () => getMapboxConfiguration().source,
  );
  ...
  const [mapboxTokenStatus, setMapboxTokenStatus] = useState<string | null>(
    null,
  );
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      ...
      (error) => {
        setLocation((current) => ({
          ...current,
          status:
            error.code === error.PERMISSION_DENIED ? "unavailable" : "degraded",
          error:
   ...
   *
   * Important:
   * - The map container gets inline dimensions so Mapbox's
   *   `.mapboxgl-map { position: relative; }` CSS cannot
   *   collapse the container.
   * - ResizeObserver keeps the WebGL canvas synchronized
   *   with the real container dimensions.
   * - map.resize() is also triggered after initial layout,
   *   style load, map load and viewport resize.
   */
  useEffect(() => {
    if (!mapboxToken || !mapNode.current) {
      ...
      setMapState(mapboxToken ? "loading" : "missing-token");
    ...
    const boot = () => {
      if (!mapNode.current || !window.mapboxgl || mapRef.current) {
        return;
      }
      ...
      if (MAP_DIAGNOSTICS_ENABLED) {
        ...
        console.info("[ride/nav] style load start", {
          styleUrl: MAPBOX_STYLE,
        });
      }
      ...
      try {
        ...
        map.on("style.load", () => {
          if (MAP_DIAGNOSTICS_ENABLED) {
            console.info("[ride/nav] style load success", {
              styleUrl: MAPBOX_STYLE,
            });
          }
        ...
        map.on("load", () => {
          ...
          if (MAP_DIAGNOSTICS_ENABLED) {
            console.info("[ride/nav] map load success", {
              ...
              canvasCssWidth: map.getCanvas().getBoundingClientRect().width,
              canvasCssHeight: map.getCanvas().getBoundingClientRect().height,
              containerWidth: map.getContainer().getBoundingClientRect().width,
              containerHeight: map.getContainer().getBoundingClientRect()
                .height,
            });
          }
    ...
    if (window.mapboxgl) {
      ...
      script.addEventListener("load", boot, { once: false });
    }
  ...
  useEffect(() => {
    if (!route || !mapRef.current || mapState !== "ready") {
      return;
    }
    ...
    const destinationFeature = {
      ...
      geometry: {
        type: "Point",
        coordinates: route.destination.center,
      },
    };
    ...
    if (riderSource) {
      riderSource.setData(currentFeature);
    } else {
  ...
  }, [location.latitude, location.longitude, mapState, route]);
        ...
        try {
          ...
          const response = await fetch(
            `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
              value.trim(),
            )}.json?autocomplete=true&limit=5${proximity}&access_token=${mapboxToken}`,
          );
          ...
          {mapMessage && (
            <div className="map-empty">
              <div
                ...
                data-testid="status-map-configuration"
              >
        ...
        <aside className="side-panel">
            ...
            {!searching &&
              ...
              searchResults.length === 0 && (
                <p
                  ...
                  data-testid="status-search-no-results"
                >
                  No places found.
                </p>
              )}
              ...
              <div className="route-details">
                {nextStep && (
                  <div className="next-maneuver">
                    ...
                    <small>
                      {formatDistance(nextStep.distanceMeters, unit)}
                    </small>
                  </div>
                )}
          ...
          <section className="panel-card" aria-label="Ride telemetry">
            ...
            <div className="speed-card">
              ...
              <div className="speed-meta">
                <strong data-testid="text-ride-mode">
                  {mode.toUpperCase()}
                </strong>
          ...
          {offRoute && (
            <div className="notice" data-testid="status-off-route">
              ...
              <span>
                Off route. Navigation remains active; choose the destination
                again to request a new route.
              </span>
            </div>
          )}
            ...
            <div className="settings-group">
              ...
              <div className="setting-row">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setDebugMode(event.target.value === "on")
                  }
            ...
            <div className="settings-group">
              ...
              <div
                className={`token-status ${
                  hasMapboxToken ? "configured" : "missing"
                }`}
              >
              ...
              <div className="install-banner">
                ...
                >
                  ...
                  <span className="fine-print">
                    Put the riding screen one tap away.
                  </span>
                </div>
      ...
      {debugOpen && (
        ...
        >
          ...
          >
            ...
            <div
              className={`debug-status ${
                hasMapboxToken && mapState === "ready" ? "ok" : "warn"
              }`}
            >
              <Map size={16} />
            ...
            <div className="debug-status">
              ...
              <div>
                ...
                <span>
                  {capabilities.isSecureContext ? "Secure" : "Not secure"}{" "}
                  context · WebGPU {capabilities.hasWebGPU ? "yes" : "no"} ·
                  WebGL {capabilities.hasWebGL ? "yes" : "no"} · Camera{" "}
            ...
            {debugMode && (
              <div className="debug-grid">
                ...
                <strong>
                  {capabilities.supportsAdvancedVision
                    ? "candidate"
                    : "not available"}
                </strong>
              </div>
